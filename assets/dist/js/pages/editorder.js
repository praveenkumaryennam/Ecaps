...
$.each(osar, function (key, value) {
	$('#ssts'+value.id).attr('checked', true);
...
$('.addp').click(function(){
	if(jQuery.type($("input[name='worktype']:checked").val()) === 'undefined'){
...
$('.placeorder').click(function(){
	...
	if(product.length > 0){
	    $('.centered').show();
	    $('.placeorder').hide();
		$.post(base_url+'orders/editorder',{orderdetails}, function(res){
			res  = JSON.parse(res);
			if(res.sts == 1){
		    	Toast.fire({
			        type: 'success',
		            ...
		            window.location.href = base_url+'/orders/view/'+orderdetails.orderid;//reload();
		        });
		    }
		});
	}else{
